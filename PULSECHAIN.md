# Switch Limit Order Operators — PulseChain
...
on-chain for profit. This guide covers PulseChain contract interaction, profit
strategies, fee modes, tax-token handling, and the order lifecycle.

**API:** `https://quote.switch.win` &nbsp;|&nbsp; **Network:** `pulsechain`
&nbsp;|&nbsp; **Chain ID:** `369`

For Robinhood Chain, use the matching [Robinhood operator guide](ROBINHOOD.md).
...
```
GET https://quote.switch.win/limit-orders?network=pulsechain&status=ACTIVE
```
...
```
GET https://quote.switch.win/limit-orders/config?network=pulsechain
```
...
> **Route scaling required:** The sell tax is applied during the `goSwitchFrom` transfer, so the pool receives less than the route's `amountIn`. If tokenIn has a sell tax, route amounts must be **pre-tax** values. See [Route Amount Scaling](#route-amount-scaling-for-sell-tax-input-feeonoutputtrue) below.
